...
import { useCrashGames } from '@/query/game';
import { paths } from '@/routes/paths';
import { useNavigate } from 'react-router-dom';

export const CasinoCrashGames = () => {
  const navigate = useNavigate();

  // 使用新的 query hook
  const { data: crashGameList = [] } = useCrashGames();

  return (
    <GameCarousel title="Crash Games" allButtonLink={`${paths.main.explore.root}?category=crash`}> 
      <div className="-mr-3 flex">
        {crashGameList &&
          crashGameList?.map((slide, index) => (
            <div
              key={index}
