...
export const CasinoSlotsGames = () => {
  const navigate = useNavigate();
 
  // const { data: slotsGameList } = useGetGameListSlots();

  const { data: slotsGameList = { data: [] as IGame[], code: 0 } } = useQuery<{ data: IGame[], code: number }>({ 
    queryKey: ['getGameListSlots'],
    queryFn: () => getCommonGameList({ type: 'slots' }),
  });

  return (
    <GameCarousel title="Slots" allButtonLink={`${paths.main.explore.root}?category=slots`}> 
      <div className="-mr-3 flex">
        {slotsGameList?.code === 0 &&
          slotsGameList?.data?.map((slide, index) => ( 
            <div
              key={index}
