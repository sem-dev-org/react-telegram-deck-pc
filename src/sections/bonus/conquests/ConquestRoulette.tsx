...
export default function ConquestRoulette({ conquest }: Props) {
  ...
  const renderDescription = useMemo(() => {
    switch (conquest.key) {
      case 'conquest_roulette_master':
        return (
          <Trans
            i18nKey="bonus:conquest_roulette_master_description"
            components={[<span key="reward" className="text-primary" />]}
            values={{ reward: formatCurrency(conquest.reward_amount) }}
          />
        );
      case 'conquest_roulette_master_30':
        return (
          <Trans
            i18nKey="bonus:conquest_roulette_master_30_description"
            components={[<span key="reward" className="text-primary" />]}
            values={{ amount: formatCurrency(30), reward: formatCurrency(conquest.reward_amount) }}
          />
        );
      default:
        return <Trans i18nKey="bonus:conquest_roulette_master_description" />;
    }
  }, [conquest]);
  return (
    <div
      ...
      onClick={() => {
        navigate(`${paths.main.explore.root}?category=roulette`);
      }}
    >

      <div className="flex items-center gap-4">
        <img src="/icons/isometric/22.svg" className="h-12 w-12" />
        <div className='flex flex-between gap-2 items-center'>
          <div className="flex flex-col">
            <p className="text-base font-bold">{t('bonus:conquest_roulette_master_title')}</p>
