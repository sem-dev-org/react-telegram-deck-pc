...
export default function ConquestBaccarat({ conquest }: Props) {
  ...
  const renderDescription = useMemo(() => {
    switch (conquest.key) {
      case 'conquest_baccarat_master':
        return (
          <Trans
            i18nKey="bonus:conquest_baccarat_master_description"
            components={[<span key="reward" className="text-primary" />]}
            values={{ reward: formatCurrency(conquest.reward_amount) }}
          />
        );
      case 'conquest_baccarat_master_50':
        return (
          <Trans
            i18nKey="bonus:conquest_baccarat_master_50_description"
            components={[<span key="reward" className="text-primary" />]}
            values={{ amount: formatCurrency(50), reward: formatCurrency(conquest.reward_amount) }}
          />
        );
      default:
        return <Trans i18nKey="bonus:conquest_baccarat_description" />;
    }
  }, [conquest]);
  ...
  return (
    <div
      ...
      style={{
        background:
          'radial-gradient(100% 157.05% at 0% 46.47%, color(display-p3 0.125 0.655 0.596 / 0.3) 0%, color(display-p3 0.200 0.200 0.200 / 0.06) 100%), color(display-p3 0.114 0.137 0.165)',
      }}
      onClick={() => {
        navigate(`${paths.main.explore.root}?category=live`);
      }}
    >
      <div className="flex items-center gap-4">
        <img src="/icons/isometric/10.svg" className="h-12 w-12" />
        <div className='flex flex-between gap-2 items-center'>
          <div className="flex flex-col">
            <p className="text-base font-bold">
              <Trans i18nKey="bonus:conquest_baccarat_master_title" />
            </p>
