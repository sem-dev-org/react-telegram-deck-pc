...
export default function ConquestBlackjack({ conquest }: Props) {
  ...
  const renderDescription = useMemo(() => {
    switch (conquest.key) {
      case 'conquest_blackjack_master':
        return (
          <Trans
            i18nKey="bonus:conquest_blackjack_master_description"
            components={[<span key="reward" className="text-primary" />]}
            values={{ reward: formatCurrency(conquest.reward_amount) }}
          />
        );
      case 'conquest_blackjack_master_40':
        return (
          <Trans
            i18nKey="bonus:conquest_blackjack_master_40_description"
            components={[<span key="reward" className="text-primary" />]}
            values={{ amount: formatCurrency(40), reward: formatCurrency(conquest.reward_amount) }}
          />
        );
      default:
        return <Trans i18nKey="bonus:conquest_blackjack_master_description" />;
    }
  }, [conquest]);
  return (
    <div
      ...
      onClick={() => {
        navigate(`${paths.main.explore.root}?category=poker`);
      }}
    >
      <div className="flex items-center gap-4">
        <img src="/icons/isometric/41.svg" className="h-12 w-12" />
        <div className='flex flex-between gap-2 items-center'>
          <div className="flex flex-col">
            <p className="text-base font-bold">{t('bonus:conquest_blackjack_master_title')}</p>
