...
import { getReferralDetail } from '@/api/referral';
import { IReferralDetail } from '@/types/referral';
...
export default function ReferralDetailsPage() {
  const { id, down_line, refer_type } = useParams();
  const [referralDetail, setReferralDetail] = useState<IReferralDetail | null>(null);
  ...
  useEffect(() => {
    setLoading(true);
    getReferralDetail({ id: id, down_line: down_line, refer_type: refer_type }).then((res) => {
                ...
                <ul>
                  ...
                  <li>
                    <a>
                      ...
                      {referralDetail?.down_line_username}
                    </a>
                  </li>
                  {referralDetail?.type === 'indirect' && (
                    <li>
                      <a>
                        <svg width="12" height="14" viewBox="0 0 12 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path
              ...
              <div className="bg-base-100 rounded-xl p-3">
                <div className="flex flex-col gap-1 py-2">
                  <div className="flex h-6 items-center justify-between px-4">
                    ...
                    <div className="text-sm font-semibold">{referralDetail?.down_line_username}</div>
                  </div>
                  <div className="flex h-6 items-center justify-between px-4">
                    <div className="text-sm">{t('referral:registrationDate')}</div>
                    <div className="text-sm font-semibold">{dayjs((referralDetail?.created_at ?? 0) * 1000).format('YYYY-MM-DD')}</div>
                  </div>
                  <div className="flex h-6 items-center justify-between px-4">
                    ...
                    <div className="bg-neutral flex h-5 items-center justify-center rounded-full px-2 text-sm font-semibold">
                      <span>VIP {referralDetail?.down_line_level_after}</span>
                    </div>
                  </div>
                  <div className="flex h-6 items-center justify-between px-4">
                    <div className="text-sm">{t('referral:referralType')}</div>
                    <div className="text-sm font-bold">{referralDetail?.refer_type}</div>
                  </div>
                  {
                    referralDetail?.refer_type === 'direct' && (
                      <div className="flex h-6 items-center justify-between px-4">
                        <div className="text-sm">{t('referral:levelReward')}</div>
                        <div className="text-sm font-bold">{referralDetail?.level_reward}</div>
                      </div>
                    )
                  }
                  <div className="flex h-6 items-center justify-between px-4">
                    <div className="text-sm">{t('referral:groupReward')}</div>
                    <div className="text-sm font-bold">
                      {formatCurrency(referralDetail?.group_reward, { decimalPoint: 8 })}
                    </div>
                  </div>
                  {/* <div className="flex h-6 items-center justify-between px-4">
                  ...
                  {/* <div className="flex h-6 items-center justify-between px-4">
                    ...
                    <div className="text-secondary-content text-sm font-semibold">
                      ...
                      {formatCurrency(referralDetail?.reward, {
                        decimalPoint: 8,
                      })}
                    </div>
                  </div> */}
